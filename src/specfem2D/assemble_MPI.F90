...
!-------------------------------------------------------------------------------------------------
 ...
 subroutine assemble_MPI_scalar_send_omp(NPROC, &
                                           buffer_send_scalar_omp,buffer_recv_scalar_omp, &
                                           num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                                           nibool_interfaces_ext_mesh, &
                                           my_neighbors_ext_mesh, &
                                           request_send_recv_omp,ninterface_acoustic,inum_interfaces_acoustic)

! non-blocking MPI send

  ! sends data
  ! note: assembling data already filled into buffer_send_scalar_omp array
  ...
  integer, dimension(num_interfaces_ext_mesh) :: nibool_interfaces_ext_mesh,my_neighbors_ext_mesh
  integer, dimension(2*num_interfaces_ext_mesh) :: request_send_recv_omp
integer, dimension(num_interfaces_ext_mesh), intent(in)  :: inum_interfaces_acoustic
  ! local parameters
  integer :: iinterface,num_interface

  request_send_recv_omp(:) = 0
  ...
  if (NPROC > 1) then
    ...
    do iinterface = 1, ninterface_acoustic
      ...
      call isend_cr(buffer_send_scalar_omp(1,num_interface), &
                    nibool_interfaces_ext_mesh(num_interface), &
                    my_neighbors_ext_mesh(num_interface), &
                    itag, &
                    request_send_recv_omp(num_interface) )

      ! receive request
      call irecv_cr(buffer_recv_scalar_omp(1,num_interface), &
                    nibool_interfaces_ext_mesh(num_interface), &
                    my_neighbors_ext_mesh(num_interface), &
                    itag, &
                    request_send_recv_omp(num_interface+num_interfaces_ext_mesh) )
    enddo

  endif

  end subroutine assemble_MPI_scalar_send_omp

!
!-------------------------------------------------------------------------------------------------
!

  subroutine assemble_MPI_scalar_write_omp(NPROC, &
                        Mesh_pointer, &
                        buffer_recv_scalar_omp,num_interfaces_ext_mesh, &
                        max_nibool_interfaces_ext_mesh, &
                        request_send_recv_omp, &
                        FORWARD_OR_ADJOINT,ninterface_acoustic,inum_interfaces_acoustic)
...
! waits for send/receiver to be completed and assembles contributions
  ...
  if (NPROC > 1) then
    ...
    do iinterface = 1, ninterface_acoustic
      num_interface = inum_interfaces_acoustic(iinterface)
      call wait_req(request_send_recv_omp(num_interface + num_interfaces_ext_mesh))
    enddo

    ! adding contributions of neighbors
    call transfer_asmbl_pot_to_device(Mesh_pointer,buffer_recv_scalar_omp,FORWARD_OR_ADJOINT)

    ! wait for communications completion (send)
    do iinterface = 1, ninterface_acoustic
      num_interface = inum_interfaces_acoustic(iinterface)
      call wait_req(request_send_recv_omp(num_interface))
    enddo

  endif

  end subroutine assemble_MPI_scalar_write_omp
...
!
  ...
  subroutine assemble_MPI_vector_send_omp(NPROC, &
                                          buffer_send_vector_omp,buffer_recv_vector_omp, &
                                          num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                                          nibool_interfaces_ext_mesh, &
                                          my_neighbors_ext_mesh, &
                                          request_send_recv_vector_omp,ninterface_elastic,inum_interfaces_elastic)

! sends data
! note: array to assemble already filled into buffer_send_vector_omp array
  ...
  integer, dimension(num_interfaces_ext_mesh) :: nibool_interfaces_ext_mesh,my_neighbors_ext_mesh
  integer, dimension(2*num_interfaces_ext_mesh) :: request_send_recv_vector_omp
  integer, dimension(num_interfaces_ext_mesh), intent(in)  :: inum_interfaces_elastic
  ! local parameters
  integer :: iinterface,num_interface
  ...
  if (NPROC > 1) then
    ...
    do iinterface = 1, ninterface_elastic
      ...
      call isend_cr(buffer_send_vector_omp(1,1,num_interface), &
                     NDIM*nibool_interfaces_ext_mesh(num_interface), &
                     my_neighbors_ext_mesh(num_interface), &
                     itag, &
                     request_send_recv_vector_omp(num_interface) )

      call irecv_cr(buffer_recv_vector_omp(1,1,num_interface), &
                     NDIM*nibool_interfaces_ext_mesh(num_interface), &
                     my_neighbors_ext_mesh(num_interface), &
                     itag, &
                     request_send_recv_vector_omp(num_interface + num_interfaces_ext_mesh ) )
    enddo

  endif

  end subroutine assemble_MPI_vector_send_omp
...
!
  ...
  subroutine assemble_MPI_vector_write_omp(NPROC,Mesh_pointer, &
                                            buffer_recv_vector_omp, &
                                            num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                                            nibool_interfaces_ext_mesh,ibool_interfaces_ext_mesh, &
                                            request_send_recv_vector_omp, &
                                            FORWARD_OR_ADJOINT,ninterface_elastic,inum_interfaces_elastic )
...
! waits for data to receive and assembles
  ...
  if (NPROC > 1) then
    ...
    do iinterface = 1, ninterface_elastic
      num_interface=inum_interfaces_elastic(iinterface)
      call wait_req(request_send_recv_vector_omp(num_interface + num_interfaces_ext_mesh))
    enddo
    ...
    do iinterface = 1, ninterface_elastic
      num_interface=inum_interfaces_elastic(iinterface)
      call wait_req(request_send_recv_vector_omp(num_interface))
    enddo

  endif

  end subroutine assemble_MPI_vector_write_omp
...
! with omp functions...
  ...
  subroutine transfer_boundary_to_omp_device(NPROC, Mesh_pointer, &
                                            buffer_recv_vector_omp, &
                                            num_interfaces_ext_mesh,max_nibool_interfaces_ext_mesh, &
                                            request_send_recv_vector_omp,ninterface_elastic,inum_interfaces_elastic )
  ...
  if (NPROC > 1) then
    ...
    ! send contributions to GPU
    call transfer_boundary_to_device_a(Mesh_pointer, buffer_recv_vector_omp, max_nibool_interfaces_ext_mesh)
  endif

  end subroutine transfer_boundary_to_omp_device
