...
void compute_kernels_ani_omp_kernel(int* ispec_is_elastic,
                                    int* d_ibool,
                                    realw* accel,
                                    realw* b_displ,
...
void compute_kernels_omp_kernel(int* ispec_is_elastic,
                                int* d_ibool,
                                realw* accel,
                                realw* b_displ,
                                realw* rho_kl,
...
extern "C"
void compute_kernels_elastic_omp_(long* Mesh_pointer,realw * deltat) 
{
    ...
    compute_kernels_omp_kernel(mp->d_ispec_is_elastic,mp->d_ibool,
...
void compute_gradient_kernel_omp(int ij,
                             int ispec,
                             realw* scalar_field,
...
void compute_kernels_acoustic_kernel_omp(int* ispec_is_acoustic,
                                     int* d_ibool,
                                     realw* rhostore,
                                     realw* kappastore,
                                     realw* d_hprime_xx,
    ...
    for(int ispec = 0; ispec < NSPEC_AB; ispec++) {
        for(int ij = 0; ij < NGLL2; ij++) {
            ...
            if (active) {
                ...
                compute_gradient_kernel_omp(ij,ispec,scalar_field_displ,b_displ_elm,
                        d_hprime_xx,
                        d_xix,d_xiz,d_gammax,d_gammaz,
                        rhol);

                // acceleration vector
                compute_gradient_kernel_omp(ij,ispec,scalar_field_accel,accel_elm,
                        d_hprime_xx,
                        d_xix,d_xiz,d_gammax,d_gammaz,
                        rhol);
...
extern "C"
void compute_kernels_acoustic_omp_(long* Mesh_pointer,realw * deltat) 
{
    Mesh* mp = (Mesh*)(*Mesh_pointer); //get mesh pointer out of fortran integer container
    ...
    //<<<grid,threads,0,mp->compute_stream>>>
    compute_kernels_acoustic_kernel_omp(mp->d_ispec_is_acoustic,
                                    mp->d_ibool,
                                    mp->d_rhostore,
                                    mp->d_kappastore,
                                    mp->d_hprime_xx,
...
void compute_kernels_hess_el_omp_kernel(int* ispec_is_elastic,
                                        int* d_ibool,
                                        realw* accel,
                                        realw* b_accel,
                                        realw* hess_kl,
...
void compute_kernels_hess_ac_omp_kernel(int* ispec_is_acoustic,
                                        int* d_ibool,
                                        realw* potential_dot_dot_acoustic,
    ...
    for(int ispec=0; ispec < NSPEC_AB; ispec++) {
        for(int ij=0; ij < NGLL2; ij++) {
            ...
            if (active) {
                ...
                // acceleration vector
                compute_gradient_kernel_omp(ij,ispec,
...
extern "C"
void compute_kernels_hess_omp_(long* Mesh_pointer,
                               ...
                               int* ACOUSTIC_SIMULATION) 
{
    ...
    if (*ELASTIC_SIMULATION) {
        //<<<grid,threads,0,mp->compute_stream>>>
        compute_kernels_hess_el_omp_kernel(mp->d_ispec_is_elastic,
                                           mp->d_ibool,
                                           mp->d_accel,
                                           mp->d_b_accel,
                                           mp->d_hess_el_kl,
    ...
    if (*ACOUSTIC_SIMULATION) {
        //<<<grid,threads,0,mp->compute_stream>>>
        compute_kernels_hess_ac_omp_kernel(mp->d_ispec_is_acoustic,
                                           mp->d_ibool,
                                           mp->d_potential_dot_dot_acoustic,
