...
subroutine compute_forces_viscoacoustic_OMP(compute_b_wavefield_arg)
        ...
        if (iphase == 1) then
            ...
            if (any_elastic) then
                if (num_fluid_solid_edges > 0) then
                    call compute_coupling_ac_el_omp(Mesh_pointer,iphase, num_fluid_solid_edges)
                endif
            endif

            if (any_poroelastic) then
                call stop_the_code('poroelastic not implemented yet in OMP mode')
            endif

            call compute_add_sources_acoustic_OMP(iphase,compute_wavefield_1,compute_wavefield_2)
        endif
    ...
    call kernel_3_acoustic_omp(Mesh_pointer,deltatover2f,b_deltatover2f,compute_wavefield_1,compute_wavefield_2)

    ! enforces free surface (zeroes potentials at free surface)
    call acoustic_enforce_free_surf_omp(Mesh_pointer,compute_wavefield_1,compute_wavefield_2)

end subroutine compute_forces_viscoacoustic_OMP
...
subroutine compute_add_sources_acoustic_OMP(iphase,compute_wavefield_1,compute_wavefield_2)
    ...
    ! forward simulations
    if (SIMULATION_TYPE == 1) call compute_add_sources_ac_omp(Mesh_pointer,iphase,it)

    ! adjoint simulations
    if (SIMULATION_TYPE == 3 .and. nadj_rec_local > 0 .and. compute_wavefield_1) &
        call add_sources_ac_sim_2_or_3_omp(Mesh_pointer,iphase, NSTEP -it + 1, nadj_rec_local,NSTEP)

    ! adjoint simulations
    if (compute_wavefield_2) call compute_add_sources_ac_s3_omp(Mesh_pointer,iphase,NSTEP -it + 1)

end subroutine compute_add_sources_acoustic_OMP
